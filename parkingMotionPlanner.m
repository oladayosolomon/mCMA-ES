...
%clear all
clearvars -except runs R j parkingSpot TotalR
close all
clc
runTimeStartTime=tic; %Start the timer for the complete run time

algo=4; %%%c =2, g= 3 e=4, 1=sqp
...
%parkingSpot=11; %Set the goal parking spot
...
navMMin=4; %Minimum number of WP considered at a time
navWPSteps=8; %Only 1 in every WPStepsNav WP are used for the navigation planning 8
...
navDetailedPlanning=1; %Toggle the use of dt as periodicity for constraint check
...
navOptions = optimoptions(  'fmincon',... %Solver
                            ...
                            'Display','iter-detailed',... %Display type
...
manMMin=4; %Minimum number of WP considered at a time 4
manWPSteps=3; %Only 1 in every WPStepsMan WP are used for the manoeuvre planning
manMMax=4; %Maximum number of WP considered at a time 8
...
manDetailedPlanning=1; %Toggle the use of dt as periodicity for constraint check
...
savePath=1; %Toggle the saving of the path plot
...
saveMotion=1; %Toggle the saving of the motion plot
...
animate=0; %Toggle the creation of a GIF animation. WARNING: It is a time consuming task
...
% Follow the WP
[navQ,navU]=motionPlanner(q0,navU0,navWP,obstacles,dims,uBounds,navqBounds,navdN,dT,dt,navMMin,navMMax,navOptions,navDetailedPlanning,navCostBound,navDispSteps,navDispOptSteps,algo);
...
% Plan the manoeuvre
[manQ,manU]=motionPlanner(manq0,manU0,manWP,manObstacles,dims,uBounds,manqBounds,mandN,dT,dt,manMMin,manMMax,manOptions,manDetailedPlanning,manCostBound,manDispSteps,manDispOptSteps,algo);
...
stack = [j,runTime, motionTime, motionLength, finalCost, strcmpi(collisionBool, 'True'), strcmpi(obsConsBool, 'True')];
R(:,runs)= stack';
end
TotalR= [TotalR;R];
end
