function [Q,U]=motionPlanner(q0,U0,WP,obstacles,dims,uBounds,comf,dN,dT,dt,MMin,MMax,options,detailedPlanning,costBound,dispSteps,dispOptSteps,algo)
...
for k=M:1:size(WP,2)
    ...
    while (~feasible && S<=min(MMax,k))
        ...
        elseif algo ==2
                fprintf("cCMAES")
            [U1(:,(k-S)*dN+1:dN*k),cost1]=ccmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun, LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));
        
        elseif algo ==3
            fprintf("gCMAES")
            [U1(:,(k-S)*dN+1:dN*k),cost1]=gcmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun, LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));
        else
            fprintf("eCMAES")
            [U1(:,(k-S)*dN+1:dN*k),cost1]=ecmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun, LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));
        end
        U=U1;
        ...
        if (k~=size(WP,2)) %Only if the current goal WP is not the last one
            ...
            elseif algo == 2
                fprintf("cCMAES")
                [U2(:,(k-S)*dN+1:dN*(k+1)),cost2]=ccmaes(U(:,(k-S)*dN+1:dN*(k+1)), costfun, consfun ,LB(:,(k-S)*dN+1:dN*(k+1)),UB(:,(k-S)*dN+1:dN*(k+1)));

            elseif algo == 3
                fprintf("gCMAES")
                [U2(:,(k-S)*dN+1:dN*(k+1)),cost2]=gcmaes(U(:,(k-S)*dN+1:dN*(k+1)), costfun, consfun ,LB(:,(k-S)*dN+1:dN*(k+1)),UB(:,(k-S)*dN+1:dN*(k+1)));

            else
                fprintf("eCMAES")
                [U2(:,(k-S)*dN+1:dN*(k+1)),cost2]=ecmaes(U(:,(k-S)*dN+1:dN*(k+1)), costfun, consfun ,LB(:,(k-S)*dN+1:dN*(k+1)),UB(:,(k-S)*dN+1:dN*(k+1)));
            end
    ...
    for S=S0:min(MMax,k)
        ...
        elseif algo ==2
                fprintf("cCMAES")
                [U3(:,(k-S)*dN+1:dN*k),cost3]=ccmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun,LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));

        elseif algo == 3
            fprintf("gCMAES")
            [U3(:,(k-S)*dN+1:dN*k),cost3]=gcmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun,LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));

         else
            fprintf("eCMAES")
            [U3(:,(k-S)*dN+1:dN*k),cost3]=ecmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun,LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));
        end
