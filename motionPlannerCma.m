...
for k=M:1:size(WP,2)
    ...
    while (~feasible && S<=min(MMax,k))
        ...
        [U1(:,(k-S)*dN+1:dN*k),cost1]=cmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun, LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));
        ...
        if (k~=size(WP,2)) %Only if the current goal WP is not the last one
            ...
            [U2(:,(k-S)*dN+1:dN*(k+1)),cost2]=cmaes(U(:,(k-S)*dN+1:dN*(k+1)), costfun, consfun ,LB(:,(k-S)*dN+1:dN*(k+1)),UB(:,(k-S)*dN+1:dN*(k+1)));
    ...
    for S=S0:min(MMax,k)
        ...
        [U3(:,(k-S)*dN+1:dN*k),cost3]=cmaes(U(:,(k-S)*dN+1:dN*k), costfun, consfun,LB(:,(k-S)*dN+1:dN*k),UB(:,(k-S)*dN+1:dN*k));
